...
namespace TicTacToe_Client
{
    public partial class Form1 : Form
    {
        ...
        private void Handler_Click(int i, int j)
        {
            if (my_turn && a[i,j] == 0 )
            {
                GetPicByPos(i, j).Image = imageList1.Images[2];
                a[i, j] = 2;
                if ( Winner() )
                {
        ...
        private void Connection()
        {
            while ( true )
            {
                try
                {
                    ...
                    if (!client.ConnectAsync("127.0.0.1", 8003).Wait(4000)) // "31.5.88.5", 1260
                        continue;
            ...
            while ( true )
            {
                try
                {
                    ...
                    if ( type == 1) // позиция
                    {
                        ...
                        GetPicByPos(nr / 10, nr % 10).Image = imageList1.Images[1];
                        a[nr / 10, nr % 10] = 1;
                        if (Winner())
                        {
                            this.Invoke(new Action(() =>
